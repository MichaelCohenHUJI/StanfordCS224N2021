...
def negSamplingLossAndGradient(
    ...
    multVec = np.dot(outVecs, centerWordVec)
    tmp = -np.ones_like(multVec)
    tmp[0] = 1
    signedVec = multVec * tmp
    sigmoidVec = sigmoid(signedVec)
    loss = -np.sum(np.log(sigmoidVec))

    gradSignedVec = -signedVec  # signed correctly
    gradSig = sigmoid(gradSignedVec)
    gradSig[0] *= -1
    gradCenterVec = np.dot(outVecs.T, gradSig)

    gradOutsideVecs = np.zeros_like(outsideVectors)
    outSignedVec = multVec
    outSignedVec[0] = -outSignedVec[0]
    outSigmoidVec = sigmoid(outSignedVec)
    outSigmoidVec[0] = -outSigmoidVec[0]
    res = np.outer(outSigmoidVec, centerWordVec)
    for i,j in enumerate(indices):
        gradOutsideVecs[j] += res[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    curCenter = centerWordVectors[word2Ind[currentCenterWord]]
    for i, word in enumerate(outsideWords):
        wordIdx = word2Ind[word]
        curLoss, curGCV, curGOV = word2vecLossAndGradient(
            curCenter, wordIdx, outsideVectors, dataset
        )

        loss += curLoss
        gradCenterVecs[word2Ind[currentCenterWord]] += curGCV
        gradOutsideVectors += curGOV
